...
class Parser:
    ...
    def statement(self) -> Optional[ASTNode]:
        ...
        if self.current_token.type == TT_KEYWORD:
            if self.current_token.value == 'use':
                ...
                # Alias for repeat loop
                return self.repeat_statement()
            elif self.current_token.value == 'unsafe':
                return self.unsafe_block()
            elif self.current_token.value == 'owned':
                return self.owned_declaration()
            elif self.current_token.value == 'arena':
                return self.arena_declaration()
            ...
            if (self.peek() and self.peek().type == TT_OPERATOR
                ...
                # Could be arena method call: name.alloc(sz) or name.reset()
                return self.arena_method_or_expr()
            else:
                return self.expression_statement()
    ...
    def unsafe_block(self) -> UnsafeBlockNode:
        """Parse `unsafe { ... }`"""
        self.advance()  # consume 'unsafe'
        self.expect(TT_DELIMITER, '{')
        body = []
        while self.current_token and not (self.current_token.type == TT_DELIMITER and self.current_token.value == '}'):
            if self.current_token.type == TT_NEWLINE:
                self.advance()
                continue
            stmt = self.statement()
            if stmt:
                body.append(stmt)
        self.expect(TT_DELIMITER, '}')
        return UnsafeBlockNode(body)

    def owned_declaration(self) -> OwnedDeclNode:
        """Parse `owned var name = expr` or `owned dec name = expr`"""
        self.advance()  # consume 'owned'
        decl_type = 'var'
        if self.current_token and self.current_token.type == TT_KEYWORD and self.current_token.value in ('var', 'dec'):
            decl_type = self.current_token.value
            self.advance()  # consume var/dec
        if not self.current_token or self.current_token.type != TT_IDENTIFIER:
            raise SyntaxError(f"Expected identifier after 'owned' at line {self.current_token.line if self.current_token else '?'}")
        var_name = self.current_token.value
        self.advance()  # consume name
        self.expect(TT_OPERATOR, '=')
        value = self.expression()
        return OwnedDeclNode(var_name, value, decl_type)

    def arena_declaration(self) -> ArenaNode:
        """Parse `arena name(size)` - create a new arena."""
        self.advance()  # consume 'arena'
        if not self.current_token or self.current_token.type != TT_IDENTIFIER:
            raise SyntaxError(f"Expected arena name at line {self.current_token.line if self.current_token else '?'}")
        name = self.current_token.value
        self.advance()  # consume name
        self.expect(TT_DELIMITER, '(')
        size_expr = self.expression()
        self.expect(TT_DELIMITER, ')')
        return ArenaNode(name, size_expr)

    def arena_method_or_expr(self):
        """Parse `name.alloc(size)` or `name.reset()` from statement level."""
        arena_name = self.current_token.value
        self.advance()  # consume name
        ...
        if method == 'alloc':
            ...
            size_expr = self.expression()
            self.expect(TT_DELIMITER, ')')
            return ArenaAllocNode(arena_name, size_expr)
        elif method == 'reset':
            self.expect(TT_DELIMITER, '(')
            self.expect(TT_DELIMITER, ')')
            return ArenaResetNode(arena_name)
        else:
            raise SyntaxError(f"Unknown method '{method}' on identifier '{arena_name}'")
    ...
    def indexed_assignment_or_expr(self):
        ...
        if self.current_token and self.current_token.type == TT_DELIMITER and self.current_token.value == ':':
            ...
            if self.current_token and not (self.current_token.type == TT_DELIMITER and self.current_token.value == ']'):
                end = self.expression()
            index = SliceNode(None, end)
        else:
            first_expr = self.expression()
            # Check if followed by ':' (slicing)
            if self.current_token and self.current_token.type == TT_DELIMITER and self.current_token.value == ':':
                self.advance()  # Skip ':'
                end = None
                if self.current_token and not (self.current_token.type == TT_DELIMITER and self.current_token.value == ']'):
                    end = self.expression()
                index = SliceNode(first_expr, end)
            else:
                index = first_expr
    ...
    def repeat_statement(self) -> RepeatNode:
        """Parse a do/til (or repeat/til) loop."""
        if self.current_token and self.current_token.value in ('repeat', 'do'):
            self.advance()
        else:
            self.expect(TT_KEYWORD, 'do')
        self.expect(TT_DELIMITER, '{')
        body = self.block()
        self.expect(TT_DELIMITER, '}')
    ...
    def primary(self) -> ASTNode:
        ...
        # Handle built-in function calls (keywords that can be called as functions)
            ...
            # Check for list/array indexing or slicing
                ...
                if self.current_token and self.current_token.type == TT_DELIMITER and self.current_token.value == ':':
                    ...
                    self.expect(TT_DELIMITER, ']')
                    return IndexNode(node, SliceNode(None, end))
                
                # Otherwise, parse start index expression
                first_expr = self.expression()
                ...
                if self.current_token and self.current_token.type == TT_DELIMITER and self.current_token.value == ':':
                    ...
                    self.expect(TT_DELIMITER, ']')
                    return IndexNode(node, SliceNode(first_expr, end))
                else:
                    self.expect(TT_DELIMITER, ']')
                    return IndexNode(node, first_expr)
            # Check for arena dot-method call used as expression: buf.alloc(64) / buf.reset()
            elif self.current_token and self.current_token.type == TT_DELIMITER and self.current_token.value == '.':
                self.advance()  # consume '.'
                if not self.current_token or self.current_token.type != TT_IDENTIFIER:
                    raise SyntaxError("Expected method name after '.'")
                ...
                if method == 'alloc':
                    ...
                    size_expr = self.expression()
                    self.expect(TT_DELIMITER, ')')
                    return ArenaAllocNode(node.name, size_expr)
                elif method == 'reset':
                    self.expect(TT_DELIMITER, '(')
                    self.expect(TT_DELIMITER, ')')
                    return ArenaResetNode(node.name)
                else:
                    raise SyntaxError(f"Unknown method '{method}' on '{node.name}'")
            return node
