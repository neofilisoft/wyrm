    ...
    def tokenize(self) -> List[Token]:
        ...
        while self.current_char is not None:
            if self.current_char.isspace():
                ...
                # // starts a comment. It is only allowed when it appears at the beginning of a line
                # (i.e., the previous token is a newline or there are no previous tokens).
                if tokens and tokens[-1].type != TT_NEWLINE:
                    raise SyntaxError(f"Unexpected comment start '//' at line {self.line}, column {self.column}")
                # Skip the comment characters until end of line
                self.skip_comment()
                # No token emitted for comment.
