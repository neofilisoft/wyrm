...
This document describes the Wyrm language design and architecture implemented in the repository. The canonical project version is stored in the [VERSION](file:///c:/Users/BEST/Desktop/wyrm/VERSION) file (currently **v2.7.0**).

---

## 1. Toolchain & Runtime

- **`wyrmc`**: The primary compiler and execution runner. Supports direct VM execution (`wyrmc run file.wyr`) and ahead-of-time compilation (`wyrmc build file.wyr`).
- **`wyrpkg`**: The project and package manager (Cargo-inspired workflow: `new`, `init`, `build`, `run`, `install`, `update`, `publish`, `remove`, `list`).
- **AOT Codegen Path**: LLVM IR compilation to native machine binaries via Clang / LLVM toolchain.
- **Bootstrap Compiler**: Native C11 / C++20 toolchain built via [bootstrap.c](file:///c:/Users/BEST/Desktop/wyrm/wyrm/scr/bootstrap.c).
- **Development Tooling**: Python driver retained strictly for experimental AST testbeds and parser validation.
...
The current bootstrap runtime is built on a dynamic tagged union `Value`:
...
- **`number`**: Double-precision floating point number (`VAL_NUMBER`, `double number;`). Specific fixed-width primitive types (`i8`..`i64`, `u8`..`u64`, `f32`, `f64`, `char`) are planned targets for direct native LLVM IR code emission rather than runtime union variants.
...
- **`var` (Mutable)**: Explicit mutable variable declaration:
  ```wyrm
  var score = 100
  score = score + 50
  ```
...
Functions are declared with the `fn` keyword. The `fn main()` function serves as the program entry point and is automatically executed on startup:
...
## 10. Memory Management (Deterministic Reference Counting & RAII Drop Glue)

Wyrm's bootstrap runtime employs a **Dynamic Value Runtime with Deterministic Reference Counting and RAII Drop Glue**, rather than a full static compile-time borrow checker:
...
1. **Deterministic Drop Glue ([val_drop](file:///c:/Users/BEST/Desktop/wyrm/wyrm/lib/wyrm_core.c#L525))**:
   ...
   - Reassigning variables or overwriting array elements automatically drops the previous value to prevent leaks.
...
4. **Arena Allocation ([WyrmArena](file:///c:/Users/BEST/Desktop/wyrm/wyrm/lib/wyrm_arena.h#L7))**:
   - High-throughput region allocation released all at once via [arena_destroy](file:///c:/Users/BEST/Desktop/wyrm/wyrm/lib/wyrm_arena.c#L47) or reset via [arena_reset](file:///c:/Users/BEST/Desktop/wyrm/wyrm/lib/wyrm_arena.c#L41):
   ```wyrm
   arena buf(1024)
   var p = buf.alloc(64)
   ```
...
## 11. Standard Library Modules (v2.6.0)
